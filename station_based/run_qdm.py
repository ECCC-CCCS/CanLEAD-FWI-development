''' Run quantile delta mapping to bias adjust single grid-cell outputs from CanLEAD-FWI to station-based observational data.
'''
...
def QM_routine(obs, hist, sim, qnts=None, realization=None, training_realization=None, 
    ...
    window=30 # size (in years) of moving window 
    ...
    mod_win = construct_moving_yearly_window(sim.sel(time=slice('1951','2100')), # start time in 1951, so that it ends in 2100 when using 30 year windows 
                                             window=window, step=step) # xc func automates construction of sliding window of size 'window' and step 'step'. Each window is stored along a new dim
    ...
    adj_win = QM.adjust(mod_win, extrapolation="constant", interp="linear") # constant extrapolation (of adj factors), with linear interpolation between adj factors
    out = unpack_moving_yearly_window(adj_win, append_ends=True) # automated unpacking of moving windows to its original structure, retains only central data (here 10 years), except for final window
...
def apply_qm(s, provider_name, data_obs=None, data_model=None, quantiles=None, high_adj=None, fwi_components=None):
    ...
    for fwi_component in fwi_components: 
        ...
        obs_data = obs_data.where(obs_data.time.dt.year.isin(years), drop=True) # drop all years with no data from index
        ...
        mod_ref = mod_ref.where(mod_ref.time.dt.year.isin(years), drop=True) # drop all years with no OBS data from index, so we have same length of data as OBS
        ...
        for r in mod_ref.coords['realization'].values: 
            QDMall[r], count_non_nans, adj_fact[r], qdm_hist = QM_routine(obs_data, mod_ref, mod_data,                                                                                                                 
                                                                          qnts=quantiles,
                                                                          realization=r, 
        ...
        nan_components[fwi_component] = count_non_nans.rename(fwi_component) # add count_non_nans to dictionary
        ...
        for var in adjusted.variables: 
            adjusted[var].attrs = data_model[var].attrs 
...
out_path = f'{fwipaths.output_data}/{version}/station_outputs/qdm_adjusted_data/' 
