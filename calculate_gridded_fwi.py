...
for e in EnsembleNumber:
    ...
    allout = fire_weather_ufunc(tas = tnoon,
                                ...
                                # as confirmed by Carr (pers. comm. 2021)
                                dry_start = "CFS", 
                                prec_thresh=1.5, # 'wet day' threshold for DMC overwintering (from Lawson and Armitage 2008)
                                dmc_dry_factor=1.2
                                )
    ...
    attrs_to_add = dict(## Admin attrs ##
                        ...
                        history = f"Generated by {os.path.basename(sys.argv[0])}", ## LV: Need to add new git repo info here (link to repo, hash tag)
                        git_id = tracking_id, 
                        ...
                        overwintering = 'The Drought Code (DC) and Duff Moisture Code (DMC) are overwintered. DC is overwintered following '\
